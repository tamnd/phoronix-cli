...
describes what users get. Pick a format with `-o`, or let phoronix choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
phoronix <command> -o table   # aligned columns for reading
phoronix <command> -o jsonl   # one JSON object per line, for piping
phoronix <command> -o json    # a single JSON array
phoronix <command> -o csv     # spreadsheet friendly
phoronix <command> -o tsv     # tab-separated
phoronix <command> -o url     # just the URL column
phoronix <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
phoronix <command> --fields id,title,url
```
...
```bash
phoronix <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
phoronix <command>            # a table, because this is a terminal
phoronix <command> | wc -l    # JSONL, because this is a pipe
```
