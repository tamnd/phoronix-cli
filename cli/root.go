// Package cli assembles the phoronix command tree from the phoronix
...
// NewApp assembles the kit application from the phoronix domain. The
...
//
// To add a command, declare it in phoronix/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := phoronix.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(phoronix.Domain{}).Register(app)
